...
// Insert a template query string into the research agent topic input.
function _researchInsertTemplate(text) {
  var el = document.getElementById("research-agent-topic");
  if (el) {
    el.value = text;
    el.focus();
  }
...
async function submitResearchManual() {
  if (!state.activeProject) return;
  const url = (document.getElementById("research-add-url")?.value || "").trim();
  if (!url) {
    alert("Enter a URL.");
    return;
  }
  const title =
    (document.getElementById("research-add-title")?.value || "").trim() || url;
  const tagsRaw = (
    document.getElementById("research-add-tags")?.value || ""
  ).trim();
  const tags = tagsRaw
    ...
    card.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:start;gap:10px">
                ...
                <div style="display:flex;gap:4px;flex-shrink:0">
                    ...
                    <button class="btn-icon" title="Edit" onclick="openEditResearchItem('${it.id}')">✎</button>
...
async function crawlResearchItem(id) {
  ...
  try {
    ...
    const text = (r && (r.fit_markdown || r.markdown || r.html || "")) || "";
...
async function runResearchAgent() {
  ...
  const numQueries = Math.max(
    ...
    Math.min(
      10,
      parseInt(
        document.getElementById("research-agent-num-queries")?.value || "4",
        10,
      ) || 4,
    ),
  );
  const numResults = Math.max(
    ...
    Math.min(
      10,
      parseInt(
        document.getElementById("research-agent-num-results")?.value || "3",
        10,
      ) || 3,
    ),
  );
  ...
  try {
    ...
    _agentLog("▶ Expanding topic into " + numQueries + " sub-queries…", "step");
    const sysExpand =
        ...
        if (autoCrawl) {
          try {
            ...
            if (!text) {
              ...
              await dbPut("research", item);
              _agentLog(
                "    ✓ crawled " + Math.round(text.length / 1024) + " KB",
                "ok",
              );
              ...
              try {
                ...
                const userSum =
                  ...
                  "\n\nContent:\n" +
                  item.fullText.slice(0, 12000);
                const summary = await _agentLlmCollect(sysSum, userSum);
                if (summary) {
                ...
                _agentLog("    ✗ summarise: " + e.message, "err");
              }
            }
          } catch (e) {
    ...
    try {
      const reportBody = await _buildAgentReport(topic, queries, newItems);
...
// ─── Per-card edit modal ─────────────────────────────────────────────────────
async function openEditResearchItem(id) {
  const item = await dbGet("research", id);
  if (!item) return;
  // stash the id on the overlay for saveResearchItemEdit to read
  const overlay = document.getElementById("modal-overlay");
  if (overlay) overlay.dataset.editResearchId = id;
  // populate fields
  const titleEl = document.getElementById("research-edit-title");
  const urlEl = document.getElementById("research-edit-url");
  const tagsEl = document.getElementById("research-edit-tags");
  const summaryEl = document.getElementById("research-edit-summary");
  if (titleEl) titleEl.value = item.title || "";
  if (urlEl) urlEl.value = item.url || "";
  if (tagsEl) tagsEl.value = (item.tags || []).join(", ");
  if (summaryEl) summaryEl.value = item.summary || "";
  showModal("modal-research-edit");
  setTimeout(() => titleEl && titleEl.focus(), 60);
}

async function saveResearchItemEdit() {
  const overlay = document.getElementById("modal-overlay");
  if (!overlay) return;
  const id = overlay.dataset.editResearchId;
  if (!id) return;
  const item = await dbGet("research", id);
  if (!item) return;
  const titleEl = document.getElementById("research-edit-title");
  ...
  const tagsEl = document.getElementById("research-edit-tags");
  const summaryEl = document.getElementById("research-edit-summary");
  item.title = (titleEl ? titleEl.value.trim() : item.title) || item.title;
  item.url = (urlEl ? urlEl.value.trim() : item.url) || item.url;
  const rawTags = tagsEl ? tagsEl.value : "";
  item.tags = rawTags
    .split(",")
    .map((t) => t.trim())
    .filter(Boolean);
  item.summary = summaryEl ? summaryEl.value.trim() : item.summary;
  await dbPut("research", item);
  closeModal();
  loadResearchBoard();
}
...
// ─── Board export ─────────────────────────────────────────────────────────────
async function exportResearchMarkdown() {
  if (!state.activeProject) return;
  const items = await dbGetByIndex(
    "research",
    "projectId",
  ...
  if (!items || !items.length) {
    alert("No research items to export.");
    return;
  }
  items.sort((a, b) => b.retrievedAt - a.retrievedAt);
  ...
  for (const it of items) {
    md += "## " + (it.title || it.url) + "\n";
    md += "**URL:** " + it.url + "\n";
    md += "**Retrieved:** " + new Date(it.retrievedAt).toLocaleString() + "\n";
  ...
  a.href = url;
  a.download = "research-" + safeName + "-" + today + ".md";
  a.click();
...
async function exportResearchCSV() {
  ...
  let csv = "Title,URL,Tags,Summary,Retrieved,Crawled,Source\n";
  for (const it of items) {
    csv +=
      [
        csvEsc(it.title || ""),
        ...
        csvEsc((it.tags || []).join("; ")),
        csvEsc(it.summary || ""),
        csvEsc(new Date(it.retrievedAt).toLocaleString()),
        csvEsc(
          it.fullText
            ? "Yes (" + Math.round(it.fullText.length / 1024) + " KB)"
            : "No",
        ),
        csvEsc(it.source || ""),
      ].join(",") + "\n";
  ...
  const a = document.createElement("a");
  a.href = url;
  a.download = "research-" + safeName + "-" + today + ".csv";
  a.click();
  URL.revokeObjectURL(url);
}
