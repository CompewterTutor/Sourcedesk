...
async function listDriveFiles() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(
        (err.error && err.error.message) || "Drive API error " + res.status,
      );
    }
...
async function getOrCreateProjectFolder(token, projectId, projectName) {
  ...
  if (!config.visibleRootFolderId) {
    config.visibleRootFolderId = await _createFolder(token, "SourceDesk", null);
  }
...
async function backupToDrive() {
  ...
  try {
    ...
    const filename =
      "sourcedesk-backup-" + new Date().toISOString().slice(0, 10) + ".json";
...
async function importSheetsQuestions(spreadsheetId, token) {
  ...
  try {
    ...
    if (!res.ok) {
      const e = await res.json().catch(() => ({}));
      throw new Error(
        (e.error && e.error.message) || "Sheets API error " + res.status,
      );
    }
    const data = await res.json();
    ...
    for (let r = 0; r < Math.min(5, rows.length); r++) {
      ...
      const qi = h.findIndex((c) => c === "question" || c === "questions");
      if (qi !== -1) {
        headerRowIdx = r;
        qIdx = qi;
        break;
    ...
    const aIdx = headers.findIndex((c) => c === "answer" || c === "answers");
    let count = 0;
...
async function exportQuestionsToSheets(token) {
  ...
  try {
    ...
    if (!createRes.ok) {
      ...
      throw new Error(
        (e.error && e.error.message) || "Sheets API error " + createRes.status,
      );
    }
    ...
    if (!updateRes.ok) {
      ...
      throw new Error(
        (e.error && e.error.message) || "Sheets API error " + updateRes.status,
      );
    }
...
async function exportToGoogleDoc(title, content, token) {
  ...
  try {
    ...
    if (!createRes.ok) {
      const e = await createRes.json().catch(() => ({}));
      throw new Error(
        (e.error && e.error.message) || "Docs API error " + createRes.status,
      );
    }
    ...
    if (!updateRes.ok) {
      const e = await updateRes.json().catch(() => ({}));
      throw new Error(
        (e.error && e.error.message) || "Docs API error " + updateRes.status,
      );
    }
