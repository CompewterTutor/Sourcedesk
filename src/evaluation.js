...
async function _renderCriteriaTab() {
  if (!state.activeProject) return;
  var criteria = await dbGetByIndex(
    "evalCriteria",
    "projectId",
  ...
  if (!criteria.length) {
    list.innerHTML =
      '<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:12px;font-style:italic">No criteria yet. Click + Add Criterion.</div>';
    return;
  }
...
async function _selectCriterion(criterionId) {
  ...
  document
    .querySelectorAll("#eval-criteria-list .sq-item")
    .forEach(function (el) {
...
async function saveCurrentCriterion() {
  ...
  if (_currentCriterion) {
    Object.assign(_currentCriterion, {
      name: name,
      weight: weight,
      maxScore: maxScore,
  ...
  document
    ...
    .forEach(function (el) {
      el.classList.toggle(
        "active",
        el.dataset.criterionId === _currentCriterion.id,
      );
    });
...
async function deleteCurrentCriterion(id) {
  if (!state.activeProject) return;
  if (
    !confirm("Delete this criterion? All scores for it will also be deleted.")
  )
    return;
  ...
  var allScores = await dbGetByIndex(
    "evalScores",
    "projectId",
    state.activeProject.id,
  );
  for (var i = 0; i < allScores.length; i++) {
    if (allScores[i].criterionId === id)
      await dbDelete("evalScores", allScores[i].id);
  }
...
async function _renderCandidatesTab() {
  if (!state.activeProject) return;
  var candidates = await dbGetByIndex(
    "evalCandidates",
    "projectId",
    state.activeProject.id,
  );
  ...
  if (!candidates.length) {
    list.innerHTML =
      '<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:12px;font-style:italic">No candidates yet. Click + Add Candidate.</div>';
    return;
  }
  ...
  candidates.forEach(function (c) {
    ...
    el.className =
      "sq-item" +
      (_currentCandidate && _currentCandidate.id === c.id ? " active" : "");
...
async function _selectCandidate(candidateId) {
  ...
  document
    .querySelectorAll("#eval-candidates-list .sq-item")
    .forEach(function (el) {
...
async function openNewCandidate() {
  ...
  if (docsEl)
    docsEl.innerHTML =
      '<div style="color:var(--text-muted);font-size:11px;font-style:italic">Save candidate first, then associate documents.</div>';
...
async function saveCurrentCandidate() {
  ...
  document
    ...
    .forEach(function (el) {
      el.classList.toggle(
        "active",
        el.dataset.candidateId === _currentCandidate.id,
      );
    });
...
async function deleteCurrentCandidate(id) {
  if (!state.activeProject) return;
  if (
    !confirm("Delete this candidate? All scores for it will also be deleted.")
  )
    return;
...
async function _renderCandidateDocPicker() {
  ...
  var projectDocs = allDocs.filter(function (d) {
    return d.docType !== "guideline";
  });

  if (!projectDocs.length) {
    docsEl.innerHTML =
      '<div style="color:var(--text-muted);font-size:11px;font-style:italic">No documents in this project yet. Upload docs in the right panel.</div>';
    return;
  }
  ...
  projectDocs.forEach(function (d) {
    ...
    row.style.cssText =
      "display:flex;align-items:center;gap:8px;cursor:pointer;font-size:12px;color:var(--text);padding:4px 0";
...
async function removeDocFromCandidate(docId) {
  if (!_currentCandidate) return;
  _currentCandidate.sourceDocIds = (
    _currentCandidate.sourceDocIds || []
  ).filter(function (id) {
    return id !== docId;
  });
...
async function _renderScorecardTab() {
  ...
  if (!criteria.length || !candidates.length) {
    ...
    area.innerHTML =
      '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--text-muted);font-size:13px;text-align:center;padding:32px;line-height:1.7">Add ' +
      what.join(" and ") +
      " to see the scorecard.</div>";
    return;
  }

  var scores = await dbGetByIndex(
    "evalScores",
    "projectId",
    state.activeProject.id,
  );
  ...
  html += "<thead><tr>";
  html +=
    "<th style=\"padding:8px 12px;text-align:left;background:var(--surface2);border:1px solid var(--border);white-space:nowrap;font-family:'DM Mono',monospace;font-weight:600;color:var(--text-dim)\">Candidate</th>";
  criteria.forEach(function (c) {
    var desc = (c.description || "").replace(/"/g, "&quot;");
    html +=
      '<th style="padding:8px 10px;text-align:center;background:var(--surface2);border:1px solid var(--border);min-width:90px;font-family:\'DM Mono\',monospace;font-weight:600;color:var(--text-dim)" title="' +
      desc +
      '">' +
      (c.name || "").replace(/</g, "&lt;") +
      '<br><span style="font-size:10px;color:var(--text-muted);font-weight:400">' +
      (c.weight || 0) +
      "% wt</span></th>";
  });
  html +=
    "<th style=\"padding:8px 12px;text-align:center;background:var(--surface2);border:1px solid var(--border);font-family:'DM Mono',monospace;font-weight:600;color:var(--accent)\">Score</th>";
  html +=
    "<th style=\"padding:8px 12px;text-align:center;background:var(--surface2);border:1px solid var(--border);font-family:'DM Mono',monospace;font-weight:600;color:var(--text-dim)\">AI Evaluate</th>";
  html += "</tr></thead>";
  ...
  candidates.forEach(function (cand) {
    ...
    criteria.forEach(function (crit) {
      ...
      if (sr && sr.score != null) {
        weightedTotal +=
          (sr.score / (crit.maxScore || 10)) * (crit.weight || 0);
        scoredCount++;
      }
    });
    ...
    html += "<tr>";
    html +=
      '<td style="padding:8px 12px;border:1px solid var(--border);color:var(--text);font-weight:500;white-space:nowrap">' +
      (cand.name || "").replace(/</g, "&lt;") +
      "</td>";
    ...
    criteria.forEach(function (crit) {
      ...
      if (sr && sr.score != null) {
        ...
        html +=
          '<td style="padding:8px 10px;border:1px solid var(--border);text-align:center;color:' +
          col +
          ';cursor:help" title="' +
          tip +
          '">' +
    ...
    html +=
      '<td style="padding:8px 12px;border:1px solid var(--border);text-align:center;font-weight:600;color:var(--accent)">' +
      scoreDisplay +
      "</td>";
    html +=
      '<td style="padding:6px 8px;border:1px solid var(--border);text-align:center">' +
      '<button class="btn-secondary" style="font-size:11px;padding:3px 10px;white-space:nowrap" id="eval-btn-' +
      cand.id +
...
async function evaluateCandidate(candidateId) {
  ...
  try {
    ...
    if (!criteria.length)
      throw new Error("No evaluation criteria defined. Add criteria first.");
    ...
    var systemPrompt =
      "You are an objective proposal evaluator. Score the provided proposal text against each criterion and return a JSON array only. Do not include any explanation, markdown, or text outside the JSON array.";
    ...
    var userMsg =
      ...
      "\n" +
      (docContent.trim() ||
        "(No source documents attached to this candidate)") +
      "\n\n" +
      "## Evaluation Criteria\n" +
      criteriaText +
      "\n\n" +
    ...
    // When using the local provider via server.js proxy, call.body is a proxy
    // envelope where the actual LLM body is a nested JSON string — we must
    // parse the inner body string to set stream:false there.
    ...
    if (typeof bodyObj.body === "string") {
      // Proxy envelope — patch the nested LLM body
      var innerBody = JSON.parse(bodyObj.body);
      innerBody.stream = false;
      innerBody.max_tokens = 1024;
    ...
    if (!jsonMatch)
      throw new Error("No JSON array found in the model response. Try again.");
    var items = JSON.parse(jsonMatch[0]);

    // Build lookup of existing scores for this candidate to avoid duplicates
    var existingScores = await dbGetByIndex(
      "evalScores",
      "candidateId",
      candidateId,
    );
...
async function exportScorecardMarkdown() {
  ...
  candidates.forEach(function (cand) {
    ...
    md +=
      "| " +
      [cand.name || ""].concat(cells).concat([scoreDisplay]).join(" | ") +
      " |\n";
  });
  md += "\n";
  ...
  // Download
  var projSlug = (state.activeProject.name || "project")
    .toLowerCase()
    .replace(/\s+/g, "-")
    .replace(/[^a-z0-9-]/g, "");
