...
(function () {
  ...
  function setMode(textarea, mode) {
    ...
    const toggleBtn = ctx.toolbar.querySelector('[data-rte-btn="toggleMode"]');
    if (toggleBtn) {
      // Show current mode so the user knows where they are;
      // clicking switches to the other mode.
      toggleBtn.textContent = mode === "rendered" ? "✏ Raw" : "👁 Preview";
      toggleBtn.title =
        mode === "rendered"
  ...
  function buildToolbar(textarea, buttons) {
    ...
    buttons.forEach((id) => {
      // Link button — works in both raw (inserts [text](url)) and rendered (execCommand createLink) mode.
      if (id === "link") {
        const btn = makeBtn(
          "🔗",
          "Insert link (works in both Raw and Preview mode)",
        );
        btn.dataset.rteBtn = "link";
        ...
        btn.addEventListener("click", () => {
          ...
          if (ctx.mode === "rendered") {
            ...
            ctx.rendered.dispatchEvent(new Event("input", { bubbles: true }));
          } else {
            // Raw mode: wrap selection as [text](url) in the textarea
            const el = ctx.textarea;
            const s = el.selectionStart;
            const e2 = el.selectionEnd;
            const selected = el.value.slice(s, e2) || "link text";
      ...
      if (id === "sep") {
        ...
        sep.style.cssText = "width:1px;background:var(--border);margin:2px 4px";
        bar.appendChild(sep);
        return;
      }
      if (id === "toggleMode") {
        const btn = makeBtn("👁 Preview", "Switch to Preview (rendered) mode");
  ...
  function markdownToHtml(md) {
    ...
    const blockTagRe = /^\s*<(h[1-6]|ul|ol|blockquote|hr|pre|table|p|div)\b/i;
    s = s
      .split(/\n{2,}/)
      .map((block) => {
