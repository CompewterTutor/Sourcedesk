...
async function _autoSaveCurrentNote() {
  ...
  if (state.currentNote.includeInContext) {
    const _pid = state.activeProject && state.activeProject.id;
    _hindsightRetainItem(
      "note:" + state.currentNote.id,
      "# Note: " + state.currentNote.title + "\n\n" + state.currentNote.content,
      ["project:" + _pid, "type:note"],
      "project:" + _pid,
    );
  }
}
...
function renderNotesList(notes) {
  ...
  sorted.forEach((n) => {
    ...
    el.className =
      "note-item" +
      (state.currentNote && state.currentNote.id === n.id ? " active" : "");
    ...
    el.querySelector(".note-pin-btn").addEventListener("click", async (e) => {
      e.stopPropagation();
...
async function searchAllNotes(query) {
  ...
  matched.forEach((n) => {
    ...
    el.onclick = async (e) => {
      ...
      if (!state.activeProject || state.activeProject.id !== n.projectId) {
...
async function saveCurrentNote() {
  ...
  const titleEl = document.getElementById("note-title-input");
  const editorEl = document.getElementById("note-editor");
  state.currentNote.title = (titleEl ? titleEl.value.trim() : "") || "Untitled";
