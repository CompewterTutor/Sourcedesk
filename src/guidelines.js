...
async function loadGuidelines() {
    ...
    const allDocs = await dbGetByIndex('docs', 'projectId', state.activeProject.id);
    const docs = allDocs.filter(function (d) { return d.docType === 'guideline'; });
    docs.sort(function (a, b) { return (b.uploadedAt || 0) - (a.uploadedAt || 0); });
...
function renderGuidelinesList(docs) {
    ...
    if (!docs || docs.length === 0) {
        list.innerHTML = '<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:12px;font-style:italic">No guideline documents yet.<br>Upload a job description or SOP.</div>';
        return;
    }
    docs.forEach(function (doc) {
        const el = document.createElement('div');
        el.className = 'sq-item' + (_currentGuideline && _currentGuideline.id === doc.id ? ' active' : '');
        ...
        el.innerHTML = '<span class="sq-item-icon" style="font-size:14px">📄</span>'
            + '<div class="sq-item-text" style="flex:1;min-width:0">'
            + '<div style="font-size:12px;color:var(--text);font-weight:500;word-break:break-word">' + (doc.name || 'Untitled').replace(/</g, '&lt;') + '</div>'
            + '<div style="font-size:11px;color:var(--text-muted)">' + date + (sizeKb ? ' · ' + sizeKb + ' KB' : '') + '</div>'
            + '</div>'
            + '<button class="btn-ghost" style="font-size:11px;padding:2px 6px;flex-shrink:0" onclick="event.stopPropagation();deleteGuideline(\'' + doc.id + '\')" title="Remove">✕</button>';

        el.onclick = function () { selectGuideline(doc.id); };
        list.appendChild(el);
    });
}
...
async function selectGuideline(id) {
    ...
    if (recsEl) {
        ...
        recsEl.innerHTML = '<div style="color:var(--text-muted);font-size:12px;white-space:pre-wrap;line-height:1.5">'
            + preview + (doc.content && doc.content.length > 300 ? '…' : '')
            + '</div>'
...
async function analyzeGuidelines() {
    ...
    const allDocs = await dbGetByIndex('docs', 'projectId', state.activeProject.id);
    const guidelines = allDocs.filter(function (d) { return d.docType === 'guideline'; });
    ...
    const systemPrompt = 'You are a procurement and operations analyst. Extract structured information from the provided position/guideline documents. Respond ONLY with a JSON object wrapped in a ```json code fence.';

    const userPrompt = 'Here are the guideline/position documents for this project:\n'
        + combined
    ...
    const apiCall = buildApiCall(systemPrompt, [{ role: 'user', content: userPrompt }]);
    ...
    if (!resp.ok) {
        const errText = await resp.text().catch(function () { return 'HTTP ' + resp.status; });
    ...
    if (!recs) throw new Error('Could not parse JSON from model response.\n\nRaw response:\n' + rawText.slice(0, 400));
...
function openGuidelinesAnalyze() {
    const recsEl = document.getElementById('guidelines-recommendations');
    if (recsEl) {
        recsEl.innerHTML = '<div style="padding:24px;text-align:center;color:var(--text-muted);font-size:13px">🤖 Analyzing… this may take a moment.</div>';
    }
    const panel = document.getElementById('guidelines-detail-panel');
    if (panel) panel.style.display = 'flex';
    const placeholder = document.getElementById('guidelines-detail-placeholder');
    ...
    analyzeGuidelines().catch(function (err) {
        if (recsEl) {
            recsEl.innerHTML = '<div style="padding:16px;color:var(--danger);font-size:12px">❌ Analysis failed: '
...
function _renderGuidelineRecommendations(recs) {
    ...
    if (recs.responsibilities && recs.responsibilities.length) {
        html += '<div class="guidelines-section">'
            ...
            + recs.responsibilities.map(function (r) {
                return '<li style="font-size:12px;color:var(--text-dim);line-height:1.5">' + String(r).replace(/</g, '&lt;') + '</li>';
            }).join('')
            + '</ul></div>';
    }
    ...
    if (recs.recommended_tasks && recs.recommended_tasks.length) {
        html += '<div class="guidelines-section">'
            ...
            + recs.recommended_tasks.map(function (t, i) {
                const pColor = t.priority === 'high' ? 'var(--danger)' : t.priority === 'low' ? 'var(--text-muted)' : 'var(--accent)';
                return '<div class="guidelines-card" data-task-index="' + i + '">'
                    + '<div style="display:flex;align-items:flex-start;gap:8px">'
                    + '<div style="flex:1">'
                    + '<div style="font-size:12px;font-weight:600;color:var(--text)">' + String(t.title || '').replace(/</g, '&lt;') + '</div>'
                    + '<div style="font-size:11px;color:var(--text-muted);margin-top:2px">' + String(t.description || '').replace(/</g, '&lt;') + '</div>'
                    + '</div>'
                    + '<span style="font-size:10px;background:var(--surface2);color:' + pColor + ';padding:2px 8px;border-radius:8px;white-space:nowrap;flex-shrink:0">' + (t.priority || 'medium') + '</span>'
                    + '</div>'
                    + '<button class="btn-secondary" style="margin-top:6px;font-size:11px;padding:3px 10px" onclick="_guidelineCreateTask(' + _guidelineJsonAttr(t) + ', this)">+ Create Task</button>'
                    + '</div>';
            }).join('')
            + '</div></div>';
    }
    ...
    if (recs.recommended_templates && recs.recommended_templates.length) {
        html += '<div class="guidelines-section">'
            ...
            + '<div style="display:flex;flex-direction:column;gap:8px;margin-top:6px">'
            + recs.recommended_templates.map(function (t) {
                return '<div class="guidelines-card">'
                    + '<div style="font-size:12px;font-weight:600;color:var(--text)">' + String(t.name || '').replace(/</g, '&lt;') + '</div>'
                    + '<div style="font-size:11px;color:var(--text-muted);margin-top:2px">' + String(t.description || '').replace(/</g, '&lt;') + '</div>'
                    + '<button class="btn-secondary" style="margin-top:6px;font-size:11px;padding:3px 10px" onclick="_guidelineCreateTemplate(' + _guidelineJsonAttr(t) + ', this)">+ Create Template</button>'
                    + '</div>';
            }).join('')
            + '</div></div>';
    }
    ...
    if (recs.procurement_categories && recs.procurement_categories.length) {
        html += '<div class="guidelines-section">'
            ...
            + recs.procurement_categories.map(function (c) {
                return '<span style="font-size:11px;background:var(--surface2);color:var(--text-dim);padding:3px 10px;border-radius:12px;border:1px solid var(--border)">' + String(c).replace(/</g, '&lt;') + '</span>';
            }).join('')
            + '</div></div>';
    }
    ...
    if (recs.reminders && recs.reminders.length) {
        html += '<div class="guidelines-section">'
            ...
            + recs.reminders.map(function (r) {
                const days = r.offset_days != null ? ' (in ' + r.offset_days + ' day' + (r.offset_days === 1 ? '' : 's') + ')' : '';
                return '<div style="font-size:12px;color:var(--text-dim);padding:6px 10px;background:var(--surface2);border-radius:var(--radius);line-height:1.5">'
                    + '<span style="font-weight:600">' + String(r.title || '').replace(/</g, '&lt;') + '</span>'
                    + '<span style="color:var(--text-muted)">' + days + '</span>'
                    + (r.description ? '<div style="font-size:11px;color:var(--text-muted);margin-top:2px">' + String(r.description).replace(/</g, '&lt;') + '</div>' : '')
                    + '</div>';
            }).join('')
            + '</div></div>';
    }
    ...
    if (!html) {
        html = '<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:12px">No recommendations extracted. Try uploading a more detailed position description.</div>';
    }
...
function _guidelineJsonAttr(obj) {
    return "JSON.parse(atob('" + btoa(unescape(encodeURIComponent(JSON.stringify(obj)))) + "'))";
}
...
async function _guidelineCreateTemplate(tmpl, btn) {
    const record = {
        ...
        category: 'Other',
        type: 'skeleton',
        content: '# ' + (tmpl.name || 'New Template') + '\n\n' + (tmpl.description || '') + '\n\n{{PLACEHOLDER}}',
        updatedAt: Date.now(),
    };
