...
async function loadTasks() {
    ...
    const tasks = await dbGetByIndex('tasks', 'projectId', state.activeProject.id);
...
function renderTaskList(tasks) {
    ...
    if (!tasks || tasks.length === 0) {
        list.innerHTML = '<div style="padding:20px;text-align:center;color:var(--text-muted);font-size:12px;font-style:italic">No tasks yet. Click + New Task to create one.</div>';
        return;
    }
    ...
    sorted.forEach(t => {
        ...
        el.className = 'sq-item' + (state.currentTask && state.currentTask.id === t.id ? ' active' : '');
        el.dataset.taskId = t.id;
        const icon = _taskStatusIcon(t.status);
        const prioColor = _taskPriorityColor(t.priority);
        const dueStr = t.dueDate ? ('<span style="font-size:10px;color:var(--text-muted);margin-left:4px">due ' + t.dueDate + '</span>') : '';
        const ctxDot = t.includeInContext ? '<span title="In context" style="color:var(--accent);font-size:10px;margin-left:4px">●</span>' : '';
...
function openNewTask() {
    ...
    // Deselect list items
    document.querySelectorAll('#task-list .sq-item').forEach(el => el.classList.remove('active'));
