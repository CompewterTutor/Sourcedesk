...
async function build() {
  ...
  if (!isDev) {
    ...
    try {
      ...
      console.error('⚠  terser not found — run `npm install` first, or use --dev for an unminified build.');
      process.exit(1);
    }
    ...
    const result = await terser.minify(js, {
      ...
      mangle: {
        ...
        reserved: [
          'showView', 'openNewProject', 'saveProject', 'openNewTemplate',
          'openEditTemplate', 'saveTemplate', 'deleteTemplate',
          'openFillTemplate', 'applyFill', 'viewTemplateContent',
          'promptAttachTemplate', 'openSettings', 'saveSettings',
          'clearAllData', 'loadProject', 'sendMessage', 'toggleRightPanel',
          'toggleDoc', 'toggleOtherProject', 'deleteDoc', 'handleDocUpload',
          'selectPill', 'selectPillByVal', 'closeModal', 'closeModalOnOverlay',
          'boot',
        ],
      },
      format: {
...
// ─── Watch mode ───────────────────────────────────────────────────────────────
function watch() {
  console.log('SourceDesk build — watching src/ for changes (Ctrl+C to stop)\n');
  ...
  try {
    ...
    console.warn('   Install chokidar and wrap this script if you need watch on Linux.');
  }
}
