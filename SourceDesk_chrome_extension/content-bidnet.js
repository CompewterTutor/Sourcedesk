...
function handleExtractQA(sendResponse) {
    try {
        ...
        if (rows.length === 0) {
            sendResponse({
                ok: false,
                error: "No question rows found on this page. Make sure you are on the Q&A list page.",
            });
            return;
        }
...
async function handleFillAnswer(payload, sendResponse) {
    ...
    try {
        ...
        // ── Answer textarea ──────────────────────────────────────────────
        var answerTA = document.querySelector(
            "#answerQuestion_answer_input_EN",
        );
        if (answerTA) {
            ...
            sendResponse({
                ok: false,
                error: "Answer textarea (#answerQuestion_answer_input_EN) not found on page.",
            });
            return;
        }
...
async function handleBatchVisibility(payload, sendResponse) {
    ...
    for (var i = 0; i < total; i++) {
        ...
        if (!answerUrl) {
            sendLog(
                "Could not build URL for Q#" + qid + ", skipping.",
                "error",
            );
            continue;
        }
        ...
        try {
            ...
            if (visSel) {
                ...
                sendLog(
                    "Visibility dropdown not found for Q#" + qid + ".",
                    "error",
                );
                continue;
            }
